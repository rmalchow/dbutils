...
public class AndOrTest {
	...
	public void testSetNullExpectNull() throws SqlException {
		...
		Assert.assertEquals("SELECT * FROM `hund` `hund_1` WHERE (`hund_1`.`id` = :id_1 AND `hund_1`.`id` = :id_2)", s.getSql());
		
		c.condition(Operator.OR, t, "id", Comparator.EQ, "3");
		Assert.assertEquals("SELECT * FROM `hund` `hund_1` WHERE (`hund_1`.`id` = :id_1 AND (`hund_1`.`id` = :id_2 OR `hund_1`.`id` = :id_3))", s.getSql());
